...
export const useSearchHotelsPage = routeLoader$(({ params, url, error }) => {
  ...
  if (!vertical) throw error(404, 'Not found')

  const query = normalizeQuery(params.query)
  const page = clampInt(params.pageNumber, 1, 9999)

  // Only hotels implemented in UI for now (flights/autos can be added later)
  if (vertical !== 'hotels') {
    return {
      vertical,
      query,
      ...
      pageSize: 20,
      totalPages: 1,
      facets: buildEmptyFacets(),
      active: parseActiveFilters(url.searchParams),
      sort: normalizeSort(url.searchParams.get('sort')),
      price: { min: null as number | null, max: null as number | null, currency: 'USD' },
    }
  }
  ...
  // Facets (computed from filtered-by-query-but-not-by-facets inventory)
  ...
  return (
    <div class="mx-auto max-w-6xl px-4 pb-24 pt-8 lg:pb-10">
      ...
      <div class="flex flex-wrap items-center gap-2 text-sm text-[color:var(--color-text-muted)]">
        ...
        <span class="text-[color:var(--color-text)] capitalize">{data.vertical}</span>
        <span class="text-[color:var(--color-text-subtle)]">/</span>
      ...
      <div class="mt-4 grid gap-4 lg:grid-cols-[1fr_auto] lg:items-end">
        <div>
          <h1 class="text-balance text-3xl font-semibold tracking-tight text-[color:var(--color-text-strong)] lg:text-4xl">
            {isHotels ? 'Hotels' : 'Search'} in {data.qHuman}
          </h1>
          ...
          <div class="mt-3 flex flex-wrap gap-2">
            ...
            {isHotels ? <span class="t-badge">Hotels vertical</span> : <span class="t-badge">Coming soon</span>}
          </div>
        </div>

        {/* Sort (heatmap: high use, top-right) */}
        {isHotels ? (
          <form method="get" action={page1Action} class="t-panel flex items-center gap-2 p-3">
            ...
            {/* keep active filters in query params */}
      ...
      {/* Active filter chips (heatmap: above results) */}
      {isHotels ? (
      ...
      <div class="mt-6 grid gap-6 lg:grid-cols-[300px_1fr] lg:items-start">
              ...
              <form method="get" action={page1Action} class="mt-4 grid gap-4">
                  ...
                  <div class="mt-3 grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <input
                        ...
                        class="mt-1 w-full rounded-xl border border-[color:var(--color-border)] bg-white px-3 py-2 text-sm outline-none focus-visible:shadow-[var(--ring-focus)]"
                        placeholder="0"
                        value={data.active.priceMin != null ? String(data.active.priceMin) : ''}
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        class="mt-1 w-full rounded-xl border border-[color:var(--color-border)] bg-white px-3 py-2 text-sm outline-none focus-visible:shadow-[var(--ring-focus)]"
                        placeholder="999"
                        value={data.active.priceMax != null ? String(data.active.priceMax) : ''}
                      />
                    </div>
                  </div>
                </div>
                  ...
                  <div class="mt-2 grid grid-cols-2 gap-2">
                    {[5, 4, 3, 2].map((s) => (
                      <label key={s} class="flex items-center gap-2 text-sm text-[color:var(--color-text)]">
                        <input type="checkbox" name="stars" value={String(s)} checked={data.active.stars.includes(s)} />
                        <span>{s}★</span>
                        <span class="text-xs text-[color:var(--color-text-muted)]">
                          ({data.facets.stars[String(s)] || 0})
                        </span>
                      </label>
                    ))}
                  </div>
                ...
                <div class="t-panel p-4">
                  <label class="flex items-center gap-2 text-sm text-[color:var(--color-text)]">
                    <input type="checkbox" name="refundable" value="1" checked={data.active.refundableOnly} />
                    <span class="font-medium">Free cancellation</span>
                  </label>
                  <div class="mt-1 text-xs text-[color:var(--color-text-muted)]">
                    Show only refundable options.
                  </div>
                </div>
                ...
                <div class="t-panel p-4">
                  ...
                  <div class="mt-2 grid gap-2">
                    {data.facets.neighborhoods.map((n) => (
                      <label key={n.name} class="flex items-center justify-between gap-3 text-sm text-[color:var(--color-text)]">
                        <span class="flex items-center gap-2">
                          <input type="checkbox" name="area" value={n.name} checked={data.active.neighborhoods.includes(n.name)} />
                          <span>{n.name}</span>
                        </span>
                ...
                <div class="t-panel p-4">
                  ...
                  <div class="mt-2 grid gap-2">
                    {data.facets.amenities.map((a) => (
                      <label key={a.name} class="flex items-center justify-between gap-3 text-sm text-[color:var(--color-text)]">
                        <span class="flex items-center gap-2">
                          <input type="checkbox" name="amenity" value={a.name} checked={data.active.amenities.includes(a.name)} />
                          <span>{a.name}</span>
                        </span>
                        <span class="text-xs text-[color:var(--color-text-muted)]">{a.count}</span>
                      </label>
                    ))}
                  </div>
                </div>
                ...
                <div class="text-xs text-[color:var(--color-text-muted)]">
                  Search pages stay noindex. Destinations + hotels pages are indexable.
                </div>
              </form>
            ) : (
              <div class="mt-4 text-sm text-[color:var(--color-text-muted)]">
                This vertical is coming soon.
              </div>
            )}
          </div>
        </aside>
        ...
        <main>
          ...
          {/* Results list */}
          <section class={isHotels ? 'mt-6' : 'mt-0'}>
            {isHotels ? (
            ...
            <div class="mt-4 grid gap-3">
              {data.results.length ? (
                data.results.map((h) => <HotelResultCard key={h.id} h={h} nights={nights} />)
              ) : (
      ...
      {isHotels && mobileFiltersOpen.value ? (
        <MobileDrawer
          title="Filters"
          onClose$={() => {
            mobileFiltersOpen.value = false
        ...
        >
          <form method="get" action={page1Action} class="grid gap-4">
            ...
            <div class="grid grid-cols-2 gap-2">
              <a
                class="t-badge flex items-center justify-center hover:bg-white"
                href={`${pathBase}/1?sort=${encodeURIComponent(data.sort)}`}
              >
                Reset
              </a>
...
export const head: DocumentHead = ({ resolveValue, url }) => {
  ...
  const verticalTitle = data.vertical === 'hotels' ? 'Hotels' : data.vertical
  const title = `${verticalTitle} in ${data.qHuman} – Page ${data.page} | Andacity Travel`
  const description = `Browse ${verticalTitle.toLowerCase()} results for ${data.qHuman}. Compare totals and policies with clarity.`

  // Search pages: always noindex
  const robots = 'noindex,follow,max-image-preview:large'

  // Canonical: clean path only (no filter params)
  const canonicalPath = `/search/${data.vertical}/${encodeURIComponent(data.query)}/${data.page}`
  const canonicalHref = new URL(canonicalPath, url.origin).href

  // OG Search PNG (unsigned default, signed optional)
  let ogImage = new URL(
    `/og/search/${data.vertical}/${encodeURIComponent(data.query)}/${data.page}.png`,
    url.origin
  ).href
  ...
  if (secret) {
    const payload: OgSearchPayload = {
      ...
      q: data.query,
      page: data.page,
      title: `${verticalTitle} search`,
      subtitle: data.qHuman,
      stats: {
        priceMin: data.price.min ?? undefined,
        priceMax: data.price.max ?? undefined,
        currency: data.price.currency,
        note: data.vertical === 'hotels' ? 'Compare totals + policies' : undefined,
      },
    }
        ...
        <div class="p-5">
          <div class="flex items-start justify-between gap-4">
            <div>
              ...
              <div class="mt-3 flex flex-wrap gap-2">
                {h.refundable ? <span class="t-badge t-badge--deal">Free cancellation</span> : <span class="t-badge">Non-refundable</span>}
            ...
            <div class="text-right">
              ...
              {total != null ? (
                <div class="mt-1 text-xs text-[color:var(--color-text-muted)]">
                  Est. total: <span class="font-medium text-[color:var(--color-text)]">{formatMoney(total, h.currency)}</span> ({nights} nights)
                </div>
              ) : (
                <div class="mt-1 text-xs text-[color:var(--color-text-muted)]">
...
const MobileDrawer = component$(({ title, onClose$, children }: MobileDrawerProps) => (
...
const parseActiveFilters = (sp: URLSearchParams): ActiveFilters => {
  ...
  // Date + party (kept as query params; canonical ignores them)
...
const renderActiveChips = (a: ActiveFilters, pathBase: string, page: number, sort: SortKey) => {
  const chips: { label: string; href: string }[] = []
  const base = `${pathBase}/${page}`
...
const computeNights = (checkIn: string | null, checkOut: string | null) => {
  ...
  // guardrail to avoid insane totals
