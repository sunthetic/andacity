...
export default component$(() => {
  ...
  return (
    <Page breadcrumbs={[
      ...
      <SearchHeaderBar
        title={`Flights from ${data.from} to ${data.to}`}
        description="Compare fares by stops, airline, and time windows with lightweight client-side filters."
      >
        <span q:slot="badges" class="t-badge">
          {filteredResults.length.toLocaleString('en-US')} results
        </span>
        <span q:slot="badges" class="t-badge">Flights search</span>
        {data.depart ? (
          <span q:slot="badges" class="t-badge">
            Depart {data.depart}{data.ret ? ` · Return ${data.ret}` : ''}
          </span>
        ) : null}
      </SearchHeaderBar>
          ...
          <section class="mt-6">
            ...
            <div class="mt-4 grid gap-3">
              {filteredResults.length ? (
                filteredResults.map((flight) => <FlightResultCard key={flight.id} flight={flight} />)
              ) : (
                <SearchEmptyState
