...
export const useSearchHotelsPage = routeLoader$(async ({ params, url }) => {
  ...
  const fallbackResults = () =>
    mapHotelsToResults(HOTELS, query, { checkIn, checkOut });
  const results = matchedCity
    ? await tryDbRead(
        () => loadHotelResultsFromDb({ query, checkIn, checkOut }),
        fallbackResults,
      )
    : fallbackResults();
  const qHuman =
...
export default component$(() => {
  ...
  const filteredResults = data.results.filter((hotel) =>
    matchesHotelFilters(hotel, values.value),
  );
          ...
          <section class="mt-6">
            ...
            <div class="mt-4 grid gap-3">
              {sortedResults.length ? (
                sortedResults.map((hotel: HotelResult) => (
...
const matchesHotelFilters = (hotel: HotelResult, values: FilterValues) => {
  ...
  if (
    ...
    !selectedPriceRanges.some((range) =>
      inHotelPriceRange(hotel.priceFrom, range),
    )
  ) {
    return false;
  }
  ...
  if (
    ...
    !selectedStarRatings.includes(String(hotel.stars))
  ) {
    return false;
  }
  ...
  if (selectedAmenities.length) {
    const normalizedAmenities = hotel.amenities.map((amenity) =>
      amenity.toLowerCase(),
    );
