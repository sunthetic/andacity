    ...
    >
      <div class="grid gap-5 lg:grid-cols-[1fr_360px] lg:items-start">
        <div>
          ...
          <p class="mt-2 max-w-[80ch] text-sm text-[color:var(--color-text-muted)] lg:text-base">
            Browse indexable car rental guides for {data.city.name}. Clear
            inclusions and policy summaries. Search pages stay noindex.
          </p>
      ...
      <section class="mt-8 t-card p-5">
        ...
        <div class="mt-3 text-sm text-[color:var(--color-text-muted)]">
          This section is your SEO payload: airport pickup norms, common car
          classes, typical pricing ranges, and booking considerations for{" "}
          {data.city.name}.
        </div>
      </section>
    </Page>
...
export const head: DocumentHead = ({ resolveValue, params, url }) => {
  ...
  const canonicalHref = new URL(buildCityHref(params.citySlug), url.origin)
    .href;
