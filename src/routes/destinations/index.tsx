/**
 * CLAUDE-UI-024 — Production Destinations page implementation.
 *
 * Approved direction from CLAUDE-UI-023 sample:
 ...
 * - /destinations?experience=<tag> filtering (canonical always /destinations)
 */
...
// ─── link builders (all target valid existing routes) ─────────────────────────
...
// ─── experience tags (derived from real bestFor fields) ───────────────────────

const EXPERIENCE_TAGS: string[] = Array.from(
  new Set(DESTINATIONS.flatMap((d) => d.bestFor)),
).sort((a, b) => a.localeCompare(b))

const buildDestinationsHref = (experience?: string | null) =>
  experience
    ? `/destinations?experience=${encodeURIComponent(experience.toLowerCase())}`
    : '/destinations'

// ─── editorial collections (derived from real bestFor tags only) ──────────────
...
export default component$(() => {
  ...
  const rawExperience = String(location.url.searchParams.get('experience') || '').trim().toLowerCase()
  ...
  const heroSubtitle = activeTag
    ? `Places tagged ${activeTag.toLowerCase()} in the Andacity destination atlas — move straight into flights, hotels, and cars.`
        ...
        <div class="mx-auto flex max-w-6xl items-center gap-2 px-4 py-3">
          ...
          {EXPERIENCE_TAGS.map((tag) => {
            ...
            return (
              <a
                key={tag}
                href={buildDestinationsHref(tag)}
                class="shrink-0 whitespace-nowrap rounded-full px-4 py-1.5 text-sm font-semibold transition focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-1"
                style={
                  isActive
                    ? 'background:var(--ui-primary);color:var(--ui-on-primary);border:1px solid transparent'
          ...
          {COLLECTIONS.map((collection) => {
            ...
            return (
              <a
                ...
                href={buildDestinationsHref(primaryTag ?? null)}
              ...
              >
                {/* Gradient header band (decorative) */}
                <div
          ...
          <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {visibleDestinations.map((d) => (
              <article
                ...
                style="background:var(--ui-surface);border:1px solid var(--ui-border);box-shadow:var(--ui-shadow-card)"
              >
                {/* Gradient header — city name + airport code (not aria-hidden) */}
                <div
                ...
                <div class="p-4">
                  ...
                  <p class="mt-1 text-xs" style="color:var(--ui-text-muted)">
                    {d.neighborhoods.length} neighborhood{d.neighborhoods.length === 1 ? '' : 's'} · from {formatMoney(d.priceFrom)}/night
                    <span aria-hidden="true"> · </span>
                    <span class="sr-only">— </span>varies by season
                  </p>

                  {/* Experience tag chips (each links to experience filter) */}
                  ...
                  {/* Primary action → destination guide (indexable) */}
                  <div class="mt-4">
                    <a
                      href={guideHref(d)}
...
export const head: DocumentHead = ({ url }) => {
  const title = 'Destinations | Andacity'
  const description =
    'Browse the places Andacity can help you plan around — destination guides with flights, hotels, and car rental links for every city.'
