...
import { clampInt } from "~/lib/search/hotels/normalize";
...
export const useHotelCanonicalSrpPage = routeLoader$(
  async ({ params, url, error }) => {
    ...
    if (!citySlug) {
      console.error(
        "[Hotels SRP] Missing citySlug in canonical URL",
        { url: url.href },
      );
    ...
    const source = await loadHotelResultsFromDb({
      ...
      return {
        matchedCity: null,
        page,
        results: [] as HotelResult[],
        totalCount: 0,
        totalPages: 1,
      };
    });

    // If the city slug doesn't match any known city, surface a 404
    if (!source.matchedCity) {
      console.error(
        "[Hotels SRP] City slug not found in canonical URL",
        { citySlug, url: url.href },
      );
      throw error(404, "Not found");
    }

    const qHuman = source.matchedCity.name;
...
export default component$(() => {
  const data = useHotelCanonicalSrpPage().value;
  ...
  const searchAgainHref = `/hotels?${searchAgainParams.toString()}`;
  ...
  return (
    <Page
      breadcrumbs={[
        ...
        { label: data.qHuman, href: `/hotels/in/${encodeURIComponent(data.citySlug)}` },
        { label: "Search results", href: `${pathBase}/1` },
      ]}
    >
      <div class="mt-4">
...
export const head: DocumentHead = ({ resolveValue, url }) => {
  const data = resolveValue(useHotelCanonicalSrpPage);
...
const buildHotelSearchStatusNotice = (
  ...
  if (state === "refreshing") {
    return {
      ...
      message:
        "Updated rates and filter changes are loading. Current results remain visible until the next result set is ready.",
    };
  }
