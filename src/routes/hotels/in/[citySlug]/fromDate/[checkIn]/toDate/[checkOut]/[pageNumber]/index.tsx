...
import {
  ...
  isValidDateRange,
  normalizeCitySlug,
} from "~/lib/search/hotels/canonical";
...
const stripDuplicateDateQueryParams = (searchParams: URLSearchParams) => {
  const sanitized = new URLSearchParams(searchParams);
  const hadCheckIn = sanitized.has("checkIn");
  const hadCheckOut = sanitized.has("checkOut");
  sanitized.delete("checkIn");
  sanitized.delete("checkOut");
  return { sanitized, changed: hadCheckIn || hadCheckOut };
};

export const onGet: RequestHandler = async ({ url, redirect }) => {
  const { sanitized, changed } = stripDuplicateDateQueryParams(url.searchParams);
  if (!changed) return;

  const qs = sanitized.toString();
  const nextPath = url.pathname.endsWith("/") ? url.pathname : `${url.pathname}/`;
  throw redirect(301, qs ? `${nextPath}?${qs}` : nextPath);
};
...
export const useSearchHotelsPage = routeLoader$(async ({ params, url, error }) => {
  const citySlug = normalizeCitySlug(String(params.citySlug || ""));
  ...
  if (!citySlug || !isIsoDate(checkIn) || !isIsoDate(checkOut) || !isValidDateRange(checkIn, checkOut) || String(page) !== String(params.pageNumber || "")) {
    throw error(404, "Not found");
  }

  const query = normalizeQuery(citySlug);
  ...
  const source = await loadHotelResultsFromDb({
    ...
    loadError =
      loadFailure instanceof Error ? loadFailure.message : "Failed to load hotel results.";

    console.error("[HotelsCanonicalSrp] Failed to load hotel results", {
...
export default component$(() => {
  ...
  const pathBase = `/hotels/in/${encodeURIComponent(data.citySlug)}/fromDate/${data.checkIn}/toDate/${data.checkOut}`;
...
export const head: DocumentHead = ({ resolveValue, url }) => {
  ...
  const canonicalPath = `/hotels/in/${encodeURIComponent(data.citySlug)}/fromDate/${data.checkIn}/toDate/${data.checkOut}/${data.page}`;
