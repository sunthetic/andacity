...
import { loadCanonicalHotelSearch } from "~/server/search/loadCanonicalHotelSearch";
...
export const useCanonicalHotelSearchPage = routeLoader$(
  async ({ status, url }) => {
    const result = await loadCanonicalHotelSearch(url);
    ...
    >
      <div class="mt-4">
        ...
        <p class="mt-2 max-w-[80ch] text-sm text-[color:var(--color-text-muted)] lg:text-base">
          Placeholder renderer for canonical hotel routes backed by the shared
          search pipeline.
        </p>
      </div>
      ...
      {"error" in data ? (
        <section class="mt-8 rounded-3xl border border-[color:var(--color-danger-border,#f1b3b8)] bg-[color:var(--color-danger-surface,#fff5f5)] p-6">
          ...
          <p class="mt-3 text-sm text-[color:var(--color-text-muted)]">
            Invalid canonical hotel routes return structured validation
            payloads.
          </p>
        ...
        <section class="mt-8 grid gap-6 lg:grid-cols-[minmax(0,18rem)_minmax(0,1fr)]">
          ...
          <div class="rounded-3xl border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-6">
            ...
            <p class="mt-3 text-sm text-[color:var(--color-text-muted)]">
              ...
              returned by the canonical hotel search loader.
            </p>
