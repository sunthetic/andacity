/**
 * CLAUDE-UI-005 — Home page sample composition.
 *
 * DEV / DESIGN-SAMPLE ONLY (rendered at /dev/ui-home). A captivating,
 * photography-first, whole-trip home page concept built entirely on the
 * `--ui-*` foundation + CLAUDE-UI-002 primitives. Renders inside the
 * production global shell (SiteHeader/SiteFooter via the root layout); this
 * component is only the page body. It does NOT replace src/routes/index.tsx.
 *
 ...
 * docs/ui-redesign/samples/HOME_PAGE_SAMPLE.md for the real-photo plan.
 */
...
import { HomeSearchModule } from "~/components/dev/home/HomeSearchModule";
import {
  HOME_DESTINATIONS,
  HOME_EDITORIAL,
  HOME_POPULAR_ROUTES,
...
const Hero = component$(() => (
  ...
  >
    ...
    <div class="mx-auto max-w-6xl px-4 pt-14 pb-10 md:px-6 md:pt-20 md:pb-14">
      ...
      <p class="mt-4 text-[11px]" style="color:rgba(255,255,255,0.6)">
        Photography direction: a single cinematic destination still (golden-hour
        coastline or skyline). Gradient stand-in shown — see sample doc.
      </p>
    </div>
  </section>
));
...
const PriceClarity = component$(() => (
  <ResponsiveSection container={false} class="mx-auto max-w-6xl px-4">
    ...
    >
      ...
      {/* Illustrative price-range visual — explicitly not a live quote */}
      <div class="rounded-2xl p-5" style="background:var(--ui-surface-muted)">
        ...
        >
          <span>$420</span>
          <span style="color:var(--ui-price)">most trips ~ $610</span>
          <span>$880</span>
        </div>
        <p class="mt-4 text-[11px]" style="color:var(--ui-text-muted)">
          Sample range for illustration only — real prices come from a live
          search.
        </p>
      </div>
    </div>
...
export const HomeSample = component$(() => (
