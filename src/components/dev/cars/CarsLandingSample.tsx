/**
 * CLAUDE-UI-017 — Car rentals landing page sample composition.
 *
 * DEV / DESIGN-SAMPLE ONLY (rendered at /dev/ui-cars). A premium, mobility-native
 * car rentals landing concept built on the `--ui-*` foundation + CLAUDE-UI-002
 * primitives. Renders inside the production global shell (SiteHeader/SiteFooter
 * via the root layout); this component is only the page body.
 *
 * The search module embeds the REAL production `CarRentalSearchCard` (surface
 * "plain", hero variant) inside a `--ui-*` frame, so the sample submits through
 * the genuine canonical car-rental search flow — no broken or fake submissions.
 *
 * Everything else (vehicle classes, airport/city pickup tiles, comparison
 * essentials, policy clarity) is ILLUSTRATIVE — see carsSampleData.ts. No live
 * prices, no supplier names, no availability, mileage, insurance, or shuttle
 * claims. Vehicle examples are described by CLASS, not inventory. City tiles link
 * to real `/car-rentals/in/{slug}` pages; airport tiles prefill the real
 * `/car-rentals` form (text only) and never auto-submit.
 *
 * It does NOT replace src/routes/car-rentals/index.tsx.
...
import {
  SAMPLE_VEHICLE_CLASSES,
  SAMPLE_CAR_CITIES,
  SAMPLE_AIRPORT_PICKUPS,
  SAMPLE_COMPARE_ESSENTIALS,
  SAMPLE_POLICY_CLARITY,
  SAMPLE_TRIP_HANDOFF,
} from "~/components/dev/cars/carsSampleData";
...
const CarsHero = component$(() => (
  <section
    class="relative isolate overflow-hidden"
    style="background-image:var(--ui-hero)"
    aria-label="Car rental search hero"
  >
    <div
    ...
    <div class="mx-auto max-w-6xl px-4 pt-10 pb-9 md:pt-14 md:pb-12">
      ...
      <div class="max-w-[60ch]">
        ...
        <p class="mt-3 max-w-[52ch] text-base" style="color:rgba(255,255,255,0.9)">
          Search airport and city pickups, set your dates and times, and compare
          the essentials clearly — then keep the rest of the journey connected.
        </p>
      ...
      {/* Search module — REAL CarRentalSearchCard inside a --ui-* frame */}
      <div
        id="car-search"
...
/* ------------------------------------------------------------------ */
/* Vehicle classes — structure only, no prices                        */
/* ------------------------------------------------------------------ */
...
const VehicleClasses = component$(() => (
  <section class="mx-auto max-w-6xl px-4 py-9">
    ...
    <div class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {SAMPLE_VEHICLE_CLASSES.map((vehicle) => (
        <a
          key={vehicle.name}
          href={vehicle.href}
    ...
    <div class="mt-5 grid gap-5 lg:grid-cols-2">
      ...
      >
        ...
        <p class="mt-1 text-[12px]" style="color:var(--ui-text-muted)">
          Tap an airport to pre-fill the search — pickup desks load from a live search.
        </p>
        <div class="mt-3 grid gap-2 sm:grid-cols-2">
          {SAMPLE_AIRPORT_PICKUPS.map((airport) => (
            <a
              key={airport.code}
              href={airport.href}
              aria-label={`Search car rentals for ${airport.city} airport`}
            ...
            >
              ...
              <span class="min-w-0">
                ...
                <span class="block text-[11px]" style="color:var(--ui-text-muted)">{airport.code} airport pickup</span>
              </span>
            </a>
          ))}
        </div>
      ...
      >
        ...
        <p class="mt-1 text-[12px]" style="color:var(--ui-text-muted)">
          Browse a city guide for desks, neighbourhoods, and road-trip ideas.
        </p>
        <div class="mt-3 flex flex-col gap-2">
          {SAMPLE_CAR_CITIES.map((city) => (
            <a
              key={city.slug}
...
const ComparisonAndPolicy = component$(() => (
  ...
  >
    ...
    <div class="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
      {/* Essentials grid — guidance, not fabricated data */}
      <div
        class="p-4"
        style="background:var(--ui-surface);border:1px solid var(--ui-border);border-radius:var(--ui-radius);box-shadow:var(--ui-shadow-card)"
      >
        <h3 class="text-sm font-bold" style="color:var(--ui-text)">
          What to weigh on every rate
        </h3>
        <div class="mt-4 grid gap-3 sm:grid-cols-2">
          {SAMPLE_COMPARE_ESSENTIALS.map((item) => (
            <div key={item.label} class="flex items-start gap-2.5">
              ...
              <div>
                <div class="text-sm font-bold" style="color:var(--ui-text)">{item.label}</div>
        ...
        <p class="mt-4 text-[11px]" style="color:var(--ui-text-muted)">
          These are the fields each live rate makes clear — actual values load from
          a real search.
        </p>
      </div>
      ...
      <div class="flex flex-col gap-3">
        {SAMPLE_POLICY_CLARITY.map((policy) => (
          <div
            key={policy.title}
            class="p-4"
            style="background:var(--ui-surface);border:1px solid var(--ui-border);border-radius:var(--ui-radius);box-shadow:var(--ui-shadow-card)"
...
export const CarsLandingSample = component$(() => (
