 ...
 * sample. Editorial and calm: a quiet, verifiable trust strip, a brand block
 * with the ThemeController, three link groups backed by real routes only,
 * and a minimal legal/bottom bar. Built on the `--ui-*` token system.
 */
import { component$ } from "@builder.io/qwik";
import { Brand } from "~/components/site/Brand";
import { ThemeController } from "~/components/ui/theme/ThemeController";
import { FOOTER_NAV, FOOTER_TRUST } from "~/components/site/siteNav";
  ...
  return (
    <footer style="background:var(--ui-surface);border-top:1px solid var(--ui-border)" aria-label="Site footer">
      ...
      <div class="mx-auto max-w-6xl px-4 py-10">
        <div class="grid gap-8 lg:grid-cols-[1.4fr_2fr]">
          ...
          <div>
            ...
            <p class="mt-3 max-w-[40ch] text-sm" style="color:var(--ui-text-muted)">
              Plan the whole trip in one place — flights, stays, and cars with
              the clarity of a great booking tool and the feel of a great trip.
            </p>
          ...
          <div class="sm:hidden">
            {FOOTER_NAV.map((col) => (
              <details key={col.title} class="border-b py-1" style="border-color:var(--ui-divider)">
                <summary
                  class="flex cursor-pointer items-center justify-between py-2 text-[14px] font-bold focus:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--ui-ring)]"
        ...
        {/* Bottom bar */}
        <div
          class="mt-10 flex flex-col gap-3 border-t pt-6 sm:flex-row sm:items-center sm:justify-between"
          style="border-color:var(--ui-divider)"
        >
          <p class="text-[12px]" style="color:var(--ui-text-muted)">
            © {year} Andacity, a{" "}
            <a href="https://sunthetic.media" class="underline-offset-2 hover:underline">
              Sunthetic Media
            </a>{" "}
            venture. All rights reserved. Prices and availability can change.
            We may earn commission from select partners.
          </p>
