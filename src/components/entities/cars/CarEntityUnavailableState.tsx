...
import type { CarEntityUnavailableStateModel } from "~/types/car-entity-page";

const toneClass = (tone: CarEntityUnavailableStateModel["tone"]) =>
  tone === "critical"
    ? "border-[color:rgba(185,28,28,0.16)] bg-[color:rgba(185,28,28,0.04)]"
    : "border-[color:rgba(180,83,9,0.18)] bg-[color:rgba(180,83,9,0.05)]";

const badgeClass = (tone: CarEntityUnavailableStateModel["tone"]) =>
  tone === "critical"
    ? "text-[color:var(--color-error,#b91c1c)]"
    : "text-[color:var(--color-warning,#92400e)]";

export const CarEntityUnavailableState = component$(
  (props: CarEntityUnavailableStateProps) => {
    return (
      <section
        class={[
...
type CarEntityUnavailableStateProps = {
  state: CarEntityUnavailableStateModel;
};
