      ...
      {props.section.type === 'checkbox' ? (
        <div class="grid gap-2">
          {props.section.options.map((option) => {
            ...
            return (
              <label key={option.value} class="flex items-center gap-2 text-sm text-[color:var(--color-text)]">
                <input
                  type="checkbox"
                  checked={checked}
                  onChange$={() => props.onCheckboxToggle$(props.section.id, option.value)}
                />
        ...
        <select
          ...
          onChange$={(_, el) => props.onSelectChange$(props.section.id, el.value)}
        >
          <option value="">{props.section.placeholder ?? 'Select an option'}</option>
          {props.section.options.map((option) => (
