...
export const DateField = component$((props: DateFieldProps) => {
  ...
  const monthAnchor = useSignal(toMonthAnchor(isoValue.value));
  ...
  useVisibleTask$(({ track }) => {
    ...
    monthAnchor.value = toMonthAnchor(nextIso);
    ...
    <div ref={rootRef} class={["relative", props.class]}>
      {props.name ? (
        <input
          type="hidden"
          name={props.name}
          value={parsed.status === "valid" ? parsed.isoValue : ""}
        ...
        <input
          ...
          onInput$={(_, inputEl) => {
            ...
            lastCommittedIso.value =
              nextParsed.status === "valid" ? nextParsed.isoValue : "";
            if (nextParsed.status === "valid") {
              monthAnchor.value = toMonthAnchor(nextParsed.isoValue);
            }

            syncCustomValidity(inputEl, nextParsed, props.required === true);
          }}
          onBlur$={(_, inputEl) => {
        ...
        >
          ...
          <div class="mt-3 grid grid-cols-7 gap-1">
            ...
            {weeks.flatMap((week) =>
              week.map((day) => {
                const isSelected =
                  parsed.status === "valid" && parsed.isoValue === day.isoValue;
                ...
                return (
                  <button
                    ...
                    onClick$={() => {
                      ...
                      monthAnchor.value = toMonthAnchor(day.isoValue);
                      pickerOpen.value = false;
                      hasBlurred.value = false;
                      syncCustomValidity(
                        inputRef.value,
                    ...
                    onKeyDown$={(event) => {
                      ...
                      const nextIsoValue = shiftIsoDate(day.isoValue, delta);
                      monthAnchor.value = toMonthAnchor(nextIsoValue);
...
const syncCustomValidity = (
  ...
  inputEl.setCustomValidity(getConstraintMessage(parsed, required));
};

const focusInitialCalendarTarget = (rootEl: HTMLElement) => {
  const initialTarget =
    rootEl.querySelector<HTMLElement>('[data-date-selected="true"]') ||
    rootEl.querySelector<HTMLElement>(
      '[data-date-in-month="true"][data-date-is-today="true"]',
    ) ||
    rootEl.querySelector<HTMLElement>('[data-date-in-month="true"]');
...
const toMonthAnchor = (value: string | null | undefined) => {
  const isoValue = normalizeIsoDate(value) || getTodayIsoValue();
  return `${isoValue.slice(0, 7)}-01`;
};
