...
export const formatSavedTravelerDisplayName = (
  traveler: Pick<
    SavedTravelerProfile,
    "firstName" | "middleName" | "lastName" | "label" | "email" | "id"
  >,
) => {
  const parts = [
  ...
  return (
    toNullableText(traveler.label) ||
    toNullableText(traveler.email) ||
    `Traveler ${traveler.id.slice(-6)}`
  );
};
